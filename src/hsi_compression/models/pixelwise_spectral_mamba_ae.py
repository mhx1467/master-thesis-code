...
from hsi_compression.models.blocks import BidirectionalMambaBlock, QuantizationProxy
...
class PixelwiseSpectralMambaAutoencoder(nn.Module):
    def __init__(
        ...
        self.attn_pool = PixelwiseSpectralAttentionPooling(d_model) if pooling == 'attention' else None
    ...
    def decode_pixels(self, z: torch.Tensor) -> torch.Tensor:
        z_q = self.quantization(z)
        x_hat = self.decoder_head(z_q)
        x_hat = self.spectral_refinement(x_hat)
        x_hat = self.output_activation(x_hat)
        return x_hat, z_q

    def _sample_pixel_indices(self, valid_mask: torch.Tensor | None, h: int, w: int, device: torch.device) -> torch.Tensor:
        num_pixels = h * w
        ...
        extra = torch.randint(0, valid.numel(), (self.pixels_per_patch - valid.numel(),), device=device)
        return torch.cat([valid, valid[extra]], dim=0)

    def _forward_train_sampled(self, x: torch.Tensor, valid_mask: torch.Tensor | None = None) -> dict[str, torch.Tensor]:
        b, c, h, w = x.shape
        ...
        for bi in range(b):
            ...
            x_hat, z_q = self.decode_pixels(z)
            x_samples.append(x_pix)
            z_samples.append(z)
            zq_samples.append(z_q)
        ...
        zq_sampled = torch.stack(zq_samples, dim=0)
        ...
        return {
            ...
            'z': rearrange(z_sampled, 'b k l -> b l k 1'),
            'z_q': rearrange(zq_sampled, 'b k l -> b l k 1'),
    ...
    def forward(self, x: torch.Tensor, valid_mask: torch.Tensor | None = None) -> dict[str, torch.Tensor]:
    ...
    @staticmethod
    def compression_ratio_proxy(input_shape: tuple[int, int, int], latent_shape: tuple[int, int, int]) -> float:
        c, h, w = input_shape
