...
if [[ $# -lt 3 ]]; then
  echo "Usage: $0 <hyperview2_root> <downstream_checkpoint> <pretrained_compressor> [epochs]" >&2
  exit 1
fi
...
CONFIG="${CONFIG:-configs/mamba/hyperview2_prisma_hierarchical_spectral_mamba_ae_latent48_rd_lambda_0_0001.yaml}"
LABELS_CSV="${LABELS_CSV:-$HV2_ROOT/HYPERVIEW2/train_gt.csv}"
EXP_NAME="${EXP_NAME:-hyperview2_prisma_hierarchical_spectral_mamba_ae_latent48_rd_lambda_0_0001}"
...
echo "Saved HYPERVIEW2 Mamba RD downstream-compression outputs to: $OUT_DIR"
