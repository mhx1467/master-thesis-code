...
def main():
    ...
    train_ds = build_dataset(dataset_root=dataset_root, split_name="train",
                             difficulty=difficulty, normalized=True,
                             return_mask=True, drop_invalid_channels=drop_invalid)
    val_ds   = build_dataset(dataset_root=dataset_root, split_name="val",
                             difficulty=difficulty, normalized=True,
                             return_mask=True, drop_invalid_channels=drop_invalid)
    ...
    model_kwargs = {k: v for k, v in model_cfg.get("model_kwargs", {}).items()
                    if k != "in_channels"}

    model = build_model(model_name=model_name, in_channels=num_input_bands,
    ...
    run_cfg   = {**cfg, "num_input_bands": num_input_bands,
                 "git_hash": get_git_short_hash(), "git_dirty": is_git_dirty()}

    def _run(logger=None):
        return fit(
            model=model, train_loader=train_loader, val_loader=val_loader,
            optimizer=optimizer, loss_fn=loss_fn, device=device, epochs=epochs,
            checkpoint_path=ckpt_path, config=cfg, logger=logger,
            scheduler=scheduler, show_progress=True,
            train_sampler=None,
    ...
    if use_wandb:
        with init_wandb(project=logging_cfg.get("project", "hsi-compression"),
                        run_name=args.run_name or exp_name, config=run_cfg) as run:
